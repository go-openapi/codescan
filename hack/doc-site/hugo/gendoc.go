...
func main() {
	...
	generateCodescanYAML(requiredGoVersion, latestRelease, versionMessage, buildTime)
	fmt.Println("==> Generated codescan.yaml")
	...
	fmt.Println("    Visit: http://localhost:1313/codescan/")
	fmt.Println()

	// Start Hugo server with both configs.
	cmd := exec.CommandContext(ctx, "hugo", "server",
		"--config", "hugo.yaml,codescan.yaml",
		"--buildDrafts",
		"--disableFastRender",
		"--navigateToChanged",
		"--bind", "0.0.0.0",
		"--port", "1313",
		"--baseURL", "http://localhost:1313/codescan/",
		"--appendPort=false",
		"--logLevel", "info",
		"--cleanDestinationDir",
	)
...
// generateCodescanYAML reads the template and writes codescan.yaml with substitutions.
func generateCodescanYAML(goVersion, latestRelease, versionMessage, buildTime string) {
	tmpl, err := os.ReadFile("codescan.yaml.template")
	if err != nil {
	...
	if err := os.WriteFile("codescan.yaml", []byte(out), 0o600); err != nil { //nolint:mnd
		fatalf("writing codescan.yaml: %v", err)
	}
}
