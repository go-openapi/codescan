...
| `SkipCompiledDependencies` | `bool` | `false` | `-skip-compiled-dependencies` | `load` | Read every dependency from source, instead of taking its types from the compiler's export data under the go/packages loader. Unset (the default since v0.36.4) costs no meaning: a dependency whose source carries annotations is read back after the load, and one that merely *declares* a type the spec carries is read at the lookup that wants it — so a `swagger:strfmt` written in a library still counts, and a model declared in an unannotated dependency keeps its doc comment and its fields. Set it for cost alone: the default is markedly faster on a warm build cache and markedly slower on a cold one, since `go list -export` compiles the closure before it can read it. A closure that does not compile is handled without this — the load falls back to source and raises `scan.compiled-dependencies`. |
