...
// parametersBlock parses ref.Comments through the grammar and returns the
// first ParametersBlock — the targeting parse lives in the grammar, so the
// test asserts discovery via the same path a builder would consume.
...
// TestParameterRefDiscovery verifies the scanner discovers standalone
// `swagger:parameters` reference markers hosted on func declarations — the
// references that wire shared parameters into operations / path-items.
...
// TestSharedParametersFixturesScanClean is a smoke test that every
// shared-parameters fixture package classifies without a scan error (e.g.
// no spurious struct-annotation conflict from prose in a doc comment). It
// guards the fixtures before the later build phases consume them.
