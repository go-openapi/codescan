...
// fakeResolver maps a doc-link reference to a Resolution from a fixed table — standing in for the real go/types
// resolver (wired in a later phase).
...
func TestClean_NoSelfMarkerWithoutResolver(t *testing.T) {
	m := mangling.NewNameMangler()
	// With no Resolver, the leading self-name is left verbatim (its exposed name is unknowable) and links are humanized
	// — the P1 / live behaviour.
	out := Clean("Widget is owned by a [Person].",
...
func TestSubstituteMarkers_RoundTrip(t *testing.T) {
	...
	// final names: Widget renamed (gizmo), Person/Order kept exposed lowercase, Gone pruned (not an emitted definition)
	// → falls back to its humanized leaf.
	...
	// self-name titleized to the renamed exposed name; field chain joined; pruned key collapsed to fallback.
