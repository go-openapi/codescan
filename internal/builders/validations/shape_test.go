...
func TestIsLegalForType_KeywordsWithoutRulesAlwaysLegal(t *testing.T) {
	// required / readOnly / deprecated / discriminator have no type constraint — they apply to any type.
...
func TestIsLegalForType_DefaultExampleEnumLegalOnAnyType(t *testing.T) {
	// default / example / enum coerce against the schema's type+format via CoerceValue / CoerceEnum, so they're always
