...
// concat_score_test.go is the K2 exploration harness for the readability
// score that gates name-identity concat names (see reduce.go concatScore and
// Options.NameConcatBudget). It is NOT a regression test and is DISABLED by
// default (t.SkipNow): it logs a ranked table for human judgement and exists
// so we can re-challenge the production score function against alternatives in
// the future.
//
...
//
// The corpus, the combinations, and the row order are all deterministic, so
...
// scoreFunc is a named candidate readability score. Add more here to compare
// them side by side in the table — every function gets its own column. The
// "weighted" candidate is the production concatScore (reduce.go), so this
// harness always re-challenges whatever ships.
...
//
// It is kept only as the cautionary baseline: its /maxWord^2 term rewards a
// single dominating long word, so it anti-correlates with intuition.
...
// combinations returns all k-subsets of [0,n) in lexicographic index order
// (deterministic).
...
func TestConcatScoreExperiment(t *testing.T) {
	...
	// concatScoreCorpus: five common English words spanning 3..12 characters,
	// so a 2/3-word concat exercises a wide spread of part lengths.
	...
	// Rank by the weighted (production) score, ascending = most readable
	// first; tie-break by concat for stability.
