...
// TestConcatScore locks the production readability score (reduce.go) that the name-identity concat budget is checked
// against.
//
// The exploration harness in concat_score_test.go (disabled) re-challenges it; this test pins the contract
// hierarchical fallback will rely on.
...
// TestResolveHierarchicalGroup locks the nested-path resolution logic the integration fixture doesn't reach:
// depth-deepening on a shared parent, and the taken-vs-shareable root distinction.
...
// TestConcatScoreVersusDefaultBudget shows the default budget cleanly separates the common short concats (accepted)
// from the long ones.
