...
func assertOperation(t *testing.T, op *oaispec.Operation, id, summary, description string, tags, scopes []string, extensions oaispec.Extensions) {
	t.Helper()
...
func assertOperationBody(t *testing.T, op *oaispec.Operation, id, summary, description string, tags, scopes []string) {
	assert.NotNil(t, op)
	assert.EqualT(t, summary, op.Summary)
	assert.EqualT(t, description, op.Description)
	assert.EqualT(t, id, op.ID)
