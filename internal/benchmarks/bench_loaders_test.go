...
// How a package graph is loaded, and what each way of loading it costs.
//
//	A — loader strategy:         go/packages vs internal/packages (ToolchainFreeLoader)
//	B — dependency types:        read from source vs taken from compiled export data
//	C — bundled export data:     a pre-computed blob, the toolchain-free route
//	D — the raw LoadMode ladder: what go/packages charges for each capability
//	E — the SHAPE of a load:     closure size, files, types, syntax — not just the cost
//
// E is not a separate rung: every rung reports it, because the cost of a load is only half of what
// distinguishes one rung from another. The state the loader work turns on — a package with complete
// types and NO AST — is invisible in a timing and obvious in a shape.
//
// This is the evidence behind the loader tables in README.md; the whole-scan figures reported there
// come from loader-benchmark, which measures released versions this package cannot see.
...
func exportDataBlob(tb testing.TB, corpus benchCorpus) fs.FS {
	...
	// The path is operator-supplied, on purpose: a blob is produced outside the repo, in a directory
	// of the operator's choosing. There is nothing to sanitise it against.
	...
	for _, corpus := range benchCorpora(t) {
		t.Run(corpus.Name, func(t *testing.T) {
			...
			for _, rung := range goPackagesLadder() {
				measureRung(t, rung, corpus, benchEnv(), true)
			}
	...
	for _, corpus := range benchCorpora(t) {
		t.Run(corpus.Name, func(t *testing.T) {
			...
			for _, rung := range coldRungs {
				env := append(benchEnv(), "GOCACHE="+t.TempDir())
...
// BenchmarkLoadStrategies is A–C as a true benchmark, for the times a like-for-like comparison
		...
		b.Run(corpus.Name, func(b *testing.B) {
			for _, rung := range loaderLadder(b, corpus) {
				b.Run(benchName(rung.name), func(b *testing.B) {
					// Warm the cache outside the timer: the first CompiledDependencies load compiles
					// the closure's export data, a one-off and not what this benchmark compares.
					_, err := rung.load(corpus, benchEnv())
					...
					b.ResetTimer()
					for range b.N {
						res, err := rung.load(corpus, benchEnv())
						require.NoError(b, err)
						b.StopTimer()
						shape = shapeOf(res.pkgs)
						b.StartTimer()
