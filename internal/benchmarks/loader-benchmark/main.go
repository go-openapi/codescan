...
// against the working tree — and run.sh builds one probe per version to put them side by side.
//
...
func run() error {
	var (
		...
		summarize, baselineLabel, gowork   string
		scanModels                         bool
		extra                              extraOptions
	)

	flag.StringVar(&dir, "dir", "", "corpus checkout to scan")
	flag.StringVar(&gowork, "gowork", "off",
		"GOWORK for the scan; \"off\" is the default because the shipped corpora unpack inside a workspace that does not list them")
	...
	flag.StringVar(&baselineLabel, "baseline-label", "v0.33.3", "label deltas are computed against")
	extra.register()
	flag.Parse()
	...
	// Through the environment rather than through Options, because this source has to compile
	// against every release being measured and Options.GOWORK is younger than some of them. The
	// go command reads it from the environment either way.
	if gowork != "" {
		if err := os.Setenv("GOWORK", gowork); err != nil {
			return fmt.Errorf("setting GOWORK: %w", err)
		}
	}
