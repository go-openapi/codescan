...
// Loader loads and type-checks Go packages without invoking the Go toolchain.
//
// A Loader is single-use per Load call in the sense that it caches nothing across calls; it is safe
// to keep one around and call Load repeatedly.
type Loader struct {
	opts *options
	vfs  *vfs.FS
}
...
// Load resolves patterns into type-checked packages, using whichever strategy the Loader was
// configured with. See [WithStrategy].
//
// The signature mirrors packages.Load so the two are interchangeable at the call site. Both
// strategies deliver the same shape — named packages with files, transitive imports, syntax and type
// information — so a caller reads the result the same way whichever one ran.
...
//
// cfg.Mode is accepted and ignored here — there is no cheaper mode on offer, so this strategy always
// produces syntax and type information.
...
//
// The build environment is resolved in three tiers, weakest first: the platform the loader runs on,
// then Config.Env (parity with packages.Load), then an explicit [WithGoEnv]. The tiering matters
// because the weakest tier is the one that lies: inside a WASI guest it says wasip1, and every
...
// buildTags extracts -tags values from GOFLAGS and BuildFlags, accepting the three spellings the go
// command does.
//
// GOFLAGS is read first because the go command treats it as a source of defaults that an explicit
// flag overrides. Tags accumulate rather than replace, which is also what the go command does for
...
//
// build.Default.ToolTags is computed at init from the configuration CODESCAN was built with, so it
// is a baseline, not an answer about the code under scan. Enabling adds a tag, "noX" removes one, and
// "none" clears the lot — which is why removals have to be honoured and not just additions.
...
//
// It decides whether a vendor directory is authoritative, so it belongs with the other inputs that
// change what gets built rather than merely how.
...
//
// A nil Config.Env means the current environment, as it does for packages.Load. It is not cosmetic:
// GOROOT and GOMODCACHE are how the resolver finds the standard library and the module cache, so an
...
type loadState struct {
	...
	// stubbable memoizes whether an import path resolves, so the synthesis pre-walk does not stat the
	// same paths once per importing file.
	stubbable map[string]bool

	// reported tracks which synthesized paths have already been announced, so a package imported from
	// fifty files is reported once.
	reported map[string]bool
...
func (ld *loadState) loadDir(dir, pkgPath string, root bool) (*Package, error) {
	...
	if err != nil {
		...
		if errors.As(err, &nogo) {
			// A constraint-excluded directory is not an error, it is simply not a package. Callers
			// discriminate on the nil *Package, which is why no sentinel is invented for it.
			return nil, nil //nolint:nilnil // "not a package" is a normal outcome, not a failure
		}

		// Anything else — two package clauses in one directory, an unparseable file, an invalid import
		// path — is reported ON the package rather than raised, which is what `go list -e` does. Failing
		// the whole load instead means one broken file anywhere under ./... yields no spec at all, and a
		// tree with a file mid-edit is the ordinary case, not the exotic one.
		//
		// go/build fills the package in as far as it got before giving up, so the files it did classify
		// are still reported. Recovering them by hand would mean reimplementing its constraint matching
	...
	// CgoFiles are part of the package too. go/build keeps them in a list of their own because they
	// need the cgo tool before they can be compiled; codescan never compiles, so for reading
	// declarations and comments they are ordinary source — and annotated types do live in them
	// (go-swagger#1096). The "C" pseudo-package they import resolves like any other unloadable
	// import, through synthesis.
	...
	for _, fn := range files {
		...
		// Comments are the payload for codescan, never skip them. Object resolution is the opposite:
		// it builds the legacy ast.Object graph that predates go/types, which does its own resolution
		// and ignores it. Nothing here reads it.
	...
	// Fabricate whatever this package selects through imports that will not resolve, so the checker
	// has something to bind those selectors to.
	ld.synthesizeFrom(syntax)

	// Only the two maps codescan reads. types.Info is pure output — every map handed in is one the
	// checker then fills for every node it visits, and Types alone was 30% of the loader's allocation.
	// Uses, Implicits, Selections, Scopes and Instances were being built in full and never consulted.
	info := &types.Info{
	...
	conf := types.Config{
		...
		// Function bodies are checked for the packages under scan: an annotated type may be declared
		// inside one (see the swagger:response in classification/operations/responses.go), and skipping
		// bodies leaves no TypesInfo.Defs entry for it, so the scanner never sees it.
		//
		// A dependency is a different matter. Nothing inside its function bodies can be reached from
		// outside it, and codescan reads a dependency only for the types its declarations expose. Since
		// dependencies are the bulk of any graph — the standard library alone dwarfs the module under
		// scan — checking their bodies was most of the work this loader did, and all of it discarded.
...
//
// go/types reports a *types.Error carrying a token.Pos; flattening it into the message alone loses
// the one field a caller can act on, and renders as "-: file:line: msg" when something later asks
// where the error was.
...
//
// A file importing "C" is read for its declarations — that is the whole of go-swagger#1096 — but the
// C pseudo-package is fabricated rather than generated, so every mention of it fails to type-check:
// "name int not exported by package C", "undefined: C.malloc", "cannot convert 1 to type C.malloc".
// None of that says anything about the code under scan, and left in place it surfaces as a warning
// telling the author their package did not type-check.
//
// The filter is by POSITION rather than by message, because the messages are many and the position is
// exact: an error inside a cgo file is one we cannot have an opinion on, since the C half of that file
// was never compiled. Errors elsewhere in the package are untouched and still reported.
...
func (i *importer) Import(importPath string) (*types.Package, error) {
	...
	// Pre-computed types beat reading source: the same answer, none of the work. A gap in the tree
	// falls through to source, and from there to synthesis.
	//
	...
	// Unresolvable: hand back an empty package rather than failing the whole check. The referencing
	// type resolves to "invalid", which the builders report at the point of use — a far more useful
