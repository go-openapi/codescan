...
// TestMalformedOverrideName covers go-swagger issue #874: a package- qualified swagger:response / swagger:model name
// (e.g. "utils.Error") is not a plain identifier.
//
// The strict override regex rejects it, so it would be silently dropped; these detectors let the scanner warn instead.
