...
func TestParse(t *testing.T) {
	...
	// It must be distinguishable from an absent block (nil) so the spec marshals `"security": []` rather than omitting the
	// key.
	...
	// go-swagger#2294: two keys under ONE sequence item mean AND — a single requirement object carrying both schemes.
