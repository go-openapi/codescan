...
func TestYAMLSpecScanner_UnmarshalSpec_Errors(t *testing.T) {
	t.Parallel()

	t.Run("no spec available", func(t *testing.T) {
		parser := new(YAMLSpecScanner)
		parser.setTitle = func(_ []string) {}
		parser.setDescription = func(_ []string) {}
		// Parse with no --- marker → no yamlSpec collected
		doc := buildRawTestComments([]string{"just text, no yaml"})
		require.NoError(t, parser.Parse(doc))

		err := parser.UnmarshalSpec(func(_ []byte) error { return nil })
		require.Error(t, err)
		require.ErrorIs(t, err, ErrParser)
	})

	t.Run("spec doesnt start with ---", func(t *testing.T) {
		parser := new(YAMLSpecScanner)
		// Manually inject yamlSpec without the --- marker
		parser.yamlSpec = []string{"summary: test"}
	...
	t.Run("invalid yaml", func(t *testing.T) {
		parser := new(YAMLSpecScanner)
		parser.yamlSpec = []string{"// ---", "// \tbad:", "//   yaml"}

		err := parser.UnmarshalSpec(func(_ []byte) error { return nil })
		require.Error(t, err)
	})

	t.Run("unmarshal callback error", func(t *testing.T) {
		parser := new(YAMLSpecScanner)
		parser.setTitle = func(_ []string) {}
		parser.setDescription = func(_ []string) {}
		...
		err := parser.UnmarshalSpec(func(_ []byte) error { return errCallback })
		require.Error(t, err)
		require.ErrorIs(t, err, errCallback)
	})
}

func TestNewYAMLSpecScanner(t *testing.T) {
	t.Parallel()

	var title, desc []string
	scanner := NewYAMLSpecScanner(
		func(lines []string) { title = lines },
		func(lines []string) { desc = lines },
	)
	...
	doc := buildRawTestComments(lines)
	require.NoError(t, scanner.Parse(doc))
	require.Equal(t, []string{"My Title."}, title)
	require.Equal(t, []string{"My description."}, desc)
}
