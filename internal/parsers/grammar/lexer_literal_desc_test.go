...
// descArg returns the folded argument text of the (first) swagger:description annotation in the token stream, or fails
// the test.
...
// TestLexer_DescriptionLiteral_PreservesMarkdown is the core contract: a `swagger:description |` block captures the
// body verbatim — indentation, a significant blank line, and markdown table pipes all survive.
func TestLexer_DescriptionLiteral_PreservesMarkdown(t *testing.T) {
	// Body lines carry the leading godoc `// ` convention space (modelled here as a leading space in each line); it is
	// dropped while author indentation beyond it — the 2-space nested list — is preserved.
...
// TestLexer_DescriptionLiteral_DashKeepsFollowingAnnotation is the regression the stage-1 literal mode exists for: a
// lone `---` in the body must not open a YAML fence and swallow the following annotation.
...
// TestLexer_DescriptionLiteral_BlankDoesNotTerminate: unlike the default Option B fold, a blank line inside the literal
// block is body, not a terminator; the block ends at the next annotation.
...
// TestLexer_DescriptionLiteral_KeywordLineIsBody: a keyword-looking line inside the block is captured as body, never
// treated as a terminator (decision 3 — no keyword sensitivity).
...
// TestLexer_DescriptionLiteral_MidLineSwaggerIsBody: only an annotation at the START of a line terminates the block; a
// `swagger:` token mid-line is prose.
...
// TestLexer_DescriptionLiteral_IndentedSwaggerStillTerminates documents the edge: the comment-prefix strip removes
// leading indentation before the annotation check, so even an indented line that BEGINS with `swagger:` terminates the
// block (you cannot hide it by indenting, e.g. in a code block).
...
// TestLexer_DescriptionLiteral_TrailingBlankClipped: bare `|` clips trailing blank lines (interior ones are kept, see
// above).
...
// TestLexer_DescriptionLiteral_EmptyBody: a marker with no body folds to an empty description rather than leaking the
// `|`.
func TestLexer_DescriptionLiteral_EmptyBody(t *testing.T) {
	assert.Equal(t, "", descArg(t, lexString(t, "swagger:description |")))
}

// TestLexer_DescriptionLiteral_PlainUnchanged: without the `|` marker the default Option B behaviour is intact — the
// body folds to the first blank line and trailing prose stays out of the description.
