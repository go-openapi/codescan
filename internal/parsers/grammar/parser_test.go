...
func TestParser_ResponseBlock_OptionalName(t *testing.T) {
	...
	// `swagger:response *` is a synonym for the bare form: no name (the builder keys it by the type name), no diagnostics.
...
func TestParser_SingleLineCommentAsDescription(t *testing.T) {
	...
	// Single-line title-shaped comment: default keeps it as title; the option moves it to the description.
	...
	// Multi-line comment: the title/description split is preserved in both modes — the option only affects single-line
	// comments.
...
func TestParser_PatternPropertiesBlock_CapturesRawPairList(t *testing.T) {
	// The whole `"<re>": <spec>, …` remainder is captured verbatim as one arg (it contains spaces/colons/commas the
	// builder parses).
...
func TestParser_DescriptionOverride_BareIsWellFormed(t *testing.T) {
	// A bare swagger:description / swagger:title is well-formed grammar (no parse diagnostic): the empty value is the
	// deliberate godoc-suppression affordance.
	// The emptiness *warning* (scan.empty-override) is the builder consumption point's job, not the parser's (design D7 /
	// §4).
...
func TestParser_DescriptionOverride_MultiLineBody(t *testing.T) {
	// Option B: lines following swagger:description fold into the description (blank-line / keyword / annotation / EOF
	// terminated), joined with "\n".
...
func TestParser_DescriptionOverride_CoexistsWithKeywords(t *testing.T) {
	// description/title dispatch through the schema family (like swagger:name), so a co-located validation keyword
	// surfaces as a Property rather than being rejected as context-invalid under a classifier block.
	b := parseString(t, "swagger:description The value.\nmaximum: 100")
	cb, ok := b.(*ClassifierBlock)
	require.True(t, ok, "expected *ClassifierBlock, got %T", b)
	assert.Equal(t, AnnDescription, cb.AnnotationKind())
...
// TestParser_ClassifierBlock_TypeWellFormed pins the relaxed swagger:type parsing (F3): a well-formed argument —
// canonical name, Go builtin, array, or an arbitrary identifier standing for a scanned-type reference — no longer
// raises a parser diagnostic; semantic resolution (and any unknown-type diagnostic) is the builder's job.
//
...
// TestParser_EnumDecl_Bare pins the relaxed bare-swagger:enum contract (F4b): a bare `swagger:enum` (no name, no inline
// values, no body) is structurally valid — it produces an EnumDeclBlock with an empty Name and raises NO parse
// diagnostic.
//
// The builder infers the enum name from the declared type and collects its consts; "no consts found" is a builder-level
// concern, not a grammar error.
...
maxLength: 64`
	...
	// UnboundBlocks now run title/desc classification too — first line ending in punctuation is title (heuristic 2).
	// Required for the schema builder's PreambleTitle path on indirectly-referenced non-annotated types (interfaces /
	// aliases).
...
// TestParser_SchemaBody_ExtensionsBlockTypedNested asserts that nested YAML mappings surface as typed map[string]any,
// not as yaml.v3's map[any]any or as a flat string.
...
// TestParser_SchemaBody_ExtensionsBlockMalformedYAMLEmitsDiagnostic asserts the new CodeInvalidYAMLExtensions code
// fires when the body fails YAML parsing.
