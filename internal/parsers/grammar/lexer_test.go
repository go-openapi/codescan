...
// lexString preprocesses a comment block (without leading // markers) and runs the full Lex pipeline.
//
...
func TestLexer_UnboundBlockClassifiesTitle(t *testing.T) {
	...
	// helpers.CollectScannerTitleDescription applied the same heuristics regardless of annotation presence, and downstream
	// consumers (e.g. the schema builder when a non-annotated interface is referenced via $ref) rely on PreambleTitle
	// being populated.
...
// TestLexer_TypeAnnotation_WellFormed pins the relaxed swagger:type lexing (F3): the grammar no longer owns a closed
// type vocabulary.
//
// Any well-formed token — a canonical name, a Go-builtin spelling, a []-prefixed array, a dot-qualified or arbitrary
// identifier (a scanned-type reference) — lexes as TYPE_REF; semantic validity is resolved by the builder.
...
func TestLexer_TrailingNonASCIIWhitespace(t *testing.T) {
	// U+00A0 NO-BREAK SPACE, U+2028 LINE SEPARATOR — TrimRightFunc with unicode.IsSpace must strip them; TrimRight on "
	// \t" alone would leave them attached.
	out := lexString(t, "swagger:strfmt uuid  ")
	require.NotEmpty(t, out)
	assert.Equal(t, TokenAnnotation, out[0].Kind)
	require.Len(t, out[0].Args, 1)
...
func TestLexer_GoDirectivesDroppedFromProse(t *testing.T) {
	...
	// `swagger:model Pet` matches the directive shape per isGoDirective in isolation — the swagger annotation check runs
	// first in lexLine, so this never reaches the directive filter at runtime.
...
func TestLexer_DirectiveDoesNotPolluteTitle(t *testing.T) {
	...
	srcWithDirective := strings.Join([]string{
		...
		"",
		"With a longer description.",
		// Simulate the post-`//` content of `//nolint:revive`. preprocessText feeds it as Raw verbatim via our lexString
		// helper: trimContentPrefix strips the leading `/` chars only on Text.
		"swagger:model Pet",
	}, "\n")
	...
	// Direct directive presence test via the public Parse() path: a CommentGroup with a //nolint line interleaved into a
	// docstring must not surface "nolint:" anywhere in Title / Description.
	srcGo := `package fake
...
func TestLexer_DirectiveMarkersDroppedFromProse(t *testing.T) {
	// go-swagger#2687 / #3007: Kubernetes marker comments must not leak into the model title/description.
	src := `package fake
...
// Note: the #3100 boundary — the inline swagger:route `+name:` parameter separator must NOT be stripped despite
// matching the marker shape — is locked end-to-end by TestCoverage_Bug3100 in the integration suite.
//
// The filter runs at Stage 3 (prose), after accumulateBodies has folded the route body into its keyword token, so the
// separator never reaches the marker check.
