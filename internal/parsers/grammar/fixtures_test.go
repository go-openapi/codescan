...
// TestFixtures_Petstore_PetField_RequiredAndPattern mirrors the Name field's docstring: required + pattern +
// min/maxLength, expressed via the alias forms ("minimum length", "maximum length") that the v1 keyword aliases
// support.
...
// TestFixtures_Petstore_ItemsPrefix_DeepNesting covers multiple levels of items.* prefix accumulation.
...
// TestFixtures_Petstore_ParametersBlock mirrors PetID with `swagger:parameters getPetById deletePet updatePet` —
// three operationID references.
...
// TestFixtures_Petstore_RouteBlock_GodocPrefixWithDeprecated covers the classic `// FooBar swagger:route ...` form plus
// an inline `Deprecated: true` and a `Responses:` raw block.
...
`)
	...
	// "List all pets" has no trailing punctuation and no internal blank, so heuristics 1/2/3 don't fire — heuristic 4
	// classifies the whole prose as Description. v1's helpers behaves the same way on the equivalent ProseLines slice.
...
// TestFixtures_Meta_TagsBlock pins the meta `Tags:` raw block (go-swagger#2655): the body is a YAML sequence of tag
// objects whose nested `externalDocs:` mapping — itself a meta-family keyword — must be absorbed as body text via
// the YAML-bodied indentation override, not terminate the block.
//
...
func TestFixtures_Meta_TagsBlock(t *testing.T) {
	...
	// The raw Tags body preserves per-line indentation: the nested externalDocs mapping is absorbed (indentation override)
	// and the list markers/depth survive, so the downstream YAML list parses.
	var tagsBody string
	var found bool
...
// TestFixtures_Meta_TosKeywordVariants exercises the trailing-dot, alias spelling ("Terms Of Service" /
// "TermsOfService" / "tos") that the meta v3 / v4 fixtures show.
...
// TestFixtures_Petstore_ParameterIn covers the `in:` keyword inside an UnboundBlock (a parameter struct field).
...
// TestFixtures_GodocLinterTrailingDot covers the trailing-dot elision the godot linter triggers across annotation
// lines.
...
// TestFixtures_AllOf_OptionalClassName covers swagger:allOf with and without the polymorphic class name (mirrors the
// docs examples in 23-classifier-grammar.md).
...
// TestFixtures_DefaultAnnotation_JSONForms covers the JsonValue branch (objects, arrays, numbers, booleans) and the
// RawValue fallback.
...
// TestFixtures_EnumDecl_BracketedHybrid covers the hybrid-list example from 23-classifier-grammar.md ("a, {x:1}, c,
// [1,2,3], …").
...
// TestFixtures_EnumDecl_MultilineBody backports the Q15 multi-line value-list body shape on swagger:enum.
...
// TestFixtures_BookingMeta_LeadingAnnotation covers the `swagger:meta` placed at the top of the comment group.
...
// TestFixtures_OperationDeprecatedAndExternalDocs covers the cross- over deprecated keyword + externalDocs raw block
// under an inline operation block.
...
// TestFixtures_DecorativeYAMLFenceInExtensions confirms decorative `--- … ---` fences around an extensions: body
// produce byte-identical behaviour with and without the fences (the v1 quirk noted in 10-shared.md and 40-lexer.md
// §5).
...
// TestFixtures_BlockBodyCrossover_KeywordsAcrossSiblings confirms `Consumes:` and `Produces:` consecutive raw blocks
// each terminate at the next sibling structural keyword, not at blank lines.
...
// TestFixtures_FullPipeline_FromCommentGroup exercises the public Parse(*ast.CommentGroup, *token.FileSet) entry —
// same as scanner does.
...
// TestFixtures_CrossSchemeKeyword_Schemes confirms the `schemes:` keyword is legal under meta, route, and operation but
// warns elsewhere (e.g. under model).
...
// TestFixtures_StrfmtKeywordAlias_MaxLen confirms that the alias "max len" / "max-len" / "maxLen" are equivalent to
// "maxLength".
...
// TestFixtures_DecimalNumberValue_Roundtrip confirms NUMBER_VALUE parses signed/unsigned decimals and fractional
// values.
...
//
// Extension.Value is YAML-typed (`any`); callers that only care about presence/equality compare on the typed value.
