...
// The dedupe layer is exercised end-to-end via Parse / ParseInto / TypedExtensions tests (yaml_test.go); these tests
// pin the helper's behaviour directly so future edits can't regress edge cases without turning red here too.
...
func TestDedupeNestedMapping(t *testing.T) {
	// duplicate type/in inside a nested SecurityDefinitions entry, plus a duplicate top-level scheme
...
func TestDedupeNoDuplicatesIsIdempotent(t *testing.T) {
	// A mapping without dups should pass through unchanged — the fast path in dedupePairs returns the original slice.
	body := []byte("alpha: 1\nbeta: 2\ngamma: 3\n")
	var got map[string]any
	if err := decodeYAMLBody(body, &got); err != nil {
		t.Fatalf("decode: %v", err)
...
func TestDedupeIntoTypedStruct(t *testing.T) {
	// ParseInto-style target — the dedupe must precede the struct decode so yaml.v3's strict check doesn't fire on the
	// duplicate.
	type sec struct {
...
func TestDedupeDistinctKindsNotMerged(t *testing.T) {
	// (Kind, Value) is the dedupe key — a string "1" and an int 1 have different scalar styles but the same Value+Kind
	// in YAML, so they DO collapse.
...
`)
	...
	// The surviving key may be quoted or unquoted depending on the yaml lib's round-trip; assert on the value.
	if len(got) != 1 {
