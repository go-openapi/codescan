...
func TestParseFlatMap(t *testing.T) {
	// Note: go.yaml.in/yaml/v3 returns map[string]any for string-keyed maps and auto-types scalars (unquoted "1.0" becomes
	// float64).
...
func TestParseNestedStructure(t *testing.T) {
	...
	// Numeric keys like `200` arrive as int keys; the outer map becomes map[any]any because not all keys are strings.
	...
	// The responses map has integer keys (200, 404), so the YAML library returns map[any]any (keys include non-strings).
...
func TestTypedExtensionsNestedYAML(t *testing.T) {
	// The case the schema builder's prior applyExtensionsRawBlock existed for: nested map / list values must arrive as
	// typed map[string]any and []any, not yaml.v3's map[any]any.
	body := "x-config:\n  enabled: true\n  threshold: 0.5\n  tags: [a, b, c]\n"
	m, err := yaml.TypedExtensions(body)
	if err != nil {
		t.Fatalf("parse: %v", err)
