...
// TestCoverage_Bug1635 asserts the resolution of go-swagger issue #1635 ("spec response generation
// without inner struct").
//
// A swagger:response (or swagger:parameters) that ANONYMOUSLY embeds a named struct marked in:body
// must treat the embed AS the body — exactly like a named `Body Foo` field — instead of
// promoting the embedded struct's fields.
//
// For responses the buggy form put the promoted fields under `headers`; for parameters it emitted
...
func TestCoverage_Bug1635(t *testing.T) {
	...
	// --- responses --- Guard rail: the named Body field form already yields a $ref body schema.
	named := doc.Responses["namedBodyResponse"].Schema
	require.NotNil(t, named)
	assert.Equal(t, bodyRef, named.Ref.String())

	// The #1635 defect: the anonymous-embed body must become a body schema, not response headers.
	...
	// --- parameters (same rule) --- Guard rail: the named Body field form yields a single body
	// parameter.
	...
	// The #1635 defect for parameters: the anonymous-embed body must become a SINGLE body parameter
	// $ref'ing the embedded struct, not one body param per promoted field.
