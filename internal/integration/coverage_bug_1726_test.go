...
// TestCoverage_Bug1726 locks the fix for go-swagger#1726 ("How to annotate
// lists?"): markdown-style bullet lists must be identified like the YAML-style
// dash form. The lexer normalises `*`/`+` bullets to the canonical `- `, so the
// relaxation propagates uniformly — both prose descriptions and value lists
// (consumes/produces, via Property.AsList) recognise them.
//
// `* item` / `+ item` are normalised to `- item` (matching gofmt's own rewrite
// of doc-comment bullets), so the asserted descriptions carry the dash form.
func TestCoverage_Bug1726(t *testing.T) {
	...
	// Asterisk bullets: identified as a list, normalised to the dash form
	// (not stripped to a run-on line, and not left as a stray `*`).
	...
	// Wide propagation: the value-list path (Produces/Consumes via AsList)
	// recognises markdown bullets too, not just prose descriptions.
