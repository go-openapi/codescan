...
// interpreted"): codescan reads a `pattern:` value VERBATIM — the backslash
// escapes `\t` / `\n` are preserved as the two characters they are, not turned
// into literal tab/newline. (The reporter's broken multi-line output came from
// go-swagger's spec->code generator, not from codescan's code->spec scan.)
