...
// TestCoverage_Bug796 locks the fix for go-swagger issue #796
// ("Definitions are generated without use of `swagger:model`"). Under
// `generate spec -m` (ScanModels), the scanner used to emit definitions
// for types that carry no `swagger:model`: a bare interface (Handler) and
// a `swagger:parameters` struct (pingParams) both leaked into
// `definitions`. The parameter struct also leaked its `in: path` line into
// the property description.
//
// The expected behaviour — locked by the golden — is that only the
// `swagger:model` type (pingResponse) is emitted; the parameter struct is
// rendered as an operation parameter (with a clean description and a real
// `in` field) and the interface is dropped entirely.
func TestCoverage_Bug796(t *testing.T) {
	...
	// The parameter struct renders as an operation parameter with a clean
	// description (no `in:` leakage) and a real `in` field.
