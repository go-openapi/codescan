...
// additionalProperties and patternProperties (object-schema keywords) cannot land on an OAS2
// SimpleSchema produced by parameters.Builder (a non-body parameter) or responses.Builder (a
// response header).
//
// Each occurrence is dropped with a CodeUnsupportedInSimpleSchema diagnostic — both keywords are
...
func TestCoverage_AdditionalProperties_SimpleSchemaSafeguard(t *testing.T) {
	...
	// The query parameter stays a clean string (the object keywords have no SimpleSchema slot to land
	// in — Parameter embeds SimpleSchema only).
	...
	// The response header likewise stays a clean string (the response is a shared definition
	// referenced by $ref from the operation).
	...
	// Two diagnostics per site (additionalProperties + patternProperties) × two sites (param +
	// header) = four; all CodeUnsupportedInSimpleSchema.
	var simple int
	for _, d := range diags {
