...
// TestCoverage_AliasEmbedInline pins the embed-handling contract: a struct that embeds an alias of
// a named type produces the same flat inline shape as a struct that embeds the named type directly.
//
// Embeds always inline properties, never `$ref` unless the embed carries `swagger:allOf`
// (internal/builders/schema/embedded.go).
...
func TestCoverage_AliasEmbedInline(t *testing.T) {
	...
	// EmbedsInterface (named interface embed) — flat with method promoted as property + the outer's
	// own field.
...
// TestCoverage_AliasEmbedAllOfOptIn pins the other half of the embed contract: when the user
// EXPLICITLY annotates an aliased embed with `swagger:allOf`, the spec produces allOf composition
// with a `$ref` to the embedded type.
//
...
func TestCoverage_AliasEmbedAllOfOptIn(t *testing.T) {
	...
	// EmbedsAliasOptIn (BaseAlias embed with swagger:allOf) → allOf: [{$ref: Base}, {properties:
	// {extra inline}}].
	//
	// The `swagger:allOf` annotation governs COMPOSITION SHAPE (allOf vs flat inline).
	// It does not preserve alias identity: the $ref target is the unaliased Base because the alias
	// BaseAlias itself is unannotated (no `swagger:model`).
	//
	// Only `swagger:model` makes an alias a first-class spec entity; without it, the aliasing is a Go
	// implementation detail and the spec carries the underlying name even inside an allOf composition.
	//
	// To preserve the alias name in allOf composition, the alias would need its own `swagger:model`
	// annotation (see EmbedsAliasModeledOptIn below for the witness).
	...
	// EmbedsDirectStructOptIn (Base direct embed with swagger:allOf) → allOf with $ref to Base.
	// Same annotation, same composition shape, regardless of whether the embed was aliased — alias
	...
	// EmbedsAliasModeledOptIn (BaseAliasModeled embed with swagger:allOf) → allOf: [{$ref:
	// BaseAliasModeled}, {properties: {extra inline}}].
	//
	// Bidirectional witness against EmbedsAliasOptIn above: same allOf composition shape, same outer
	// struct, but the embedded alias is ANNOTATED.
	// The annotation surfaces the alias name in the $ref target — `swagger:model` is the SOLE knob
