...
// TestCoverage_AliasResponseShapes_Default — Q4 / Q5 exploration goldens.
//
// After Q4's fix to responses.buildFieldAlias (parity with parameters), default mode dissolves an
// alias chain on a body field down to the canonical decl ($ref → Envelope, no EnvelopeAlias{,2}
// pollution in definitions).
//
// Headers typed as an alias of a named primitive now also surface correctly as SimpleSchema
...
func TestCoverage_AliasResponseShapes_Default(t *testing.T) {
	...
	// Body aliased through a chain: dissolves to canonical Envelope under default mode (no per-alias
	...
	// Bidirectional companion — the annotated alias preserves its identity at the body response
	// site; the unannotated alias above dissolves to canonical.
	...
	assert.Equal(t, "#/definitions/EnvelopeAliasModeled", modeledRef,
		"annotated body response alias keeps the alias name in the $ref")
...
// TestCoverage_AliasResponseShapes_Ref — under RefAliases the body $ref chains through
// EnvelopeAlias2 → EnvelopeAlias → Envelope; headers STILL expand inline (no $ref) because $ref
// is not legal on a SimpleSchema target.
//
...
// TestCoverage_AliasResponseShapes_Transparent — TransparentAliases dissolves every layer up
// front; the body $ref points at Envelope, headers carry their primitive type, and the
