...
// TestCoverage_Bug413 locks the fix for go-swagger issue #413 ("error reading
// embedded struct"): embedding an external-package struct that itself carries a
// custom-typed field (maps.NearbySearchRequest with a RankBy field) used to fail
// the scan with "unable to resolve embedded struct for: RankBy". It now resolves
// cleanly — the embedded fields are promoted as parameters.
