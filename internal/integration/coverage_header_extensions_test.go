...
// TestCoverage_HeaderExtensions pins M2's Walker.Extension wiring on the response-header path.
//
...
func TestCoverage_HeaderExtensions(t *testing.T) {
	...
	// Response is registered at the doc level and the operation references it via $ref — the
	// path-level operation has no inline headers, so inspect the top-level definition.
	...
	// Extensions present on the header.
	// The YAML body of the Extensions block was parsed via grammar's typed-extensions pipeline, so the
	// values arrive pre-typed (string + bool).
