...
// TestQuirk_GofmtOperationYAML is the operations counterpart of quirk F7 (TestQuirk_GofmtMetaYAML).
//
// A swagger:operation YAML body in its gofmt-canonical shape — column-0 top-level keys rendered
// as prose (one leading space), their value blocks rendered as tab-indented code blocks, separated
// by blank "//" lines — must parse identically to the space-indented form.
//
// Previously the tab indentation reached the YAML sub-parser, which rejects tabs, and the nesting
// collapsed: `responses` degraded to a single `default` with an empty description and the vendor
// extension became `null`.
//
// F7 fixed the swagger:meta case (a uniformly tab-prefixed body); this asserts the operation case,
...
func TestQuirk_GofmtOperationYAML(t *testing.T) {
	...
	// The vendor extension is preserved with its nested map (incl. responses), not dropped to null.
