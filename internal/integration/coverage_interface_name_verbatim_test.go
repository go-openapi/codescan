...
// TestCoverage_InterfaceNameVerbatim pins the Q9 contract: when the user provides `swagger:name X`
// on an interface method, X is emitted verbatim — the auto-jsonify mangler is bypassed.
//
// The default-path method (no swagger:name) is asserted alongside to triangulate against the
// mangler's lowercase-first-camelCase behaviour: any future regression that runs the mangler over
...
func TestCoverage_InterfaceNameVerbatim(t *testing.T) {
	...
	// Each swagger:name override must reach the spec verbatim.
	// If any of these fails, the mangler is running over user input — the exact regression Q9's
	// resolution forbids.
