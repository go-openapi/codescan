...
// It maps RFC 6901 JSON pointers that codescan emits via OnProvenance to the Go source position that produced them,
// and back.
// codescan anchors only code-detail nodes, so this is NOT a bijection: a finer pointer resolves to its nearest
// anchored ancestor (PositionFor), and a source line resolves to its nearest enclosing anchor (PointerAt).
...
// lineAnchor is one (1-based source line → pointer) entry within a file, for the reverse source→spec lookup.
...
//
// Backs the spec pane's gutter: the caller maps each to its rendered line, marking the nodes whose source position is
// exact.
func (x *SourceIndex) AnchoredPointers() []string {
	if x == nil {
		return nil
	}
	out := make([]string, 0, len(x.fwd))
	for ptr := range x.fwd {
		out = append(out, ptr)
	}
...
func (x *SourceIndex) AnchorLines(file string) map[int]bool {
	...
	out := make(map[int]bool, len(anchors))
