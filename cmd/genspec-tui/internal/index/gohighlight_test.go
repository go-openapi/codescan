...
// 0-based lines: 0 package 2 annotation 3 prose 4 type 5 tagged field 6 plain field 9 func with a predeclared const.
...
func TestGoHighlight_ClassifiesTokens(t *testing.T) {
	...
	// A struct tag is a string literal; the field NAME and its type stay plain, because identifiers are the bulk of Go
	// source and need something to contrast against.
...
// A block comment or raw string covers lines that contain no token of their own.
//
// Without a run starting at their column 1 they would render plain, and the closing line would go plain up to whatever
// token follows.
...
// go/token counts columns in BYTES; the renderer slices in RUNES.
//
...
// The buffer is a file someone is halfway through editing.
//
// A highlighter that gives up on incomplete syntax goes blank exactly when you are typing.
...
// Runs must ascend, or the renderer takes one back past the previous one and paints the line wrong.
...
// The scanner synthesises the semicolons the source does not write.
//
...
// Only a LEADING keyword counts, and only one the table knows.
//
// Prose that merely contains a colon is still prose.
