...
// file is a highlighter that goes blank exactly when you are typing.
//
...
func scanGo(src []byte) []goToken {
	...
	for {
		...
		// The scanner synthesises the semicolons Go's grammar requires but the source does not write.
		// Marking a run at a character that is not there would style the padding past the end of the line.
...
// addGoToken records the runs one token contributes, which is more than one when the token spans lines.
//
// A raw string or a block comment covers every line it crosses, and a continuation line carries no token of its own.
// Without a run starting at its column 1 it would render plain, and the closing line would go plain up to whatever
// token follows the comment.
func addGoToken(byLine map[int][]theme.Span, t goToken, annotated bool) {
	...
	for i, segment := range segments {
		// A continuation line of an empty segment is a BLANK line inside the comment or string.
		// It has no character to mark, and a run there would style the padding the pane fills the row with.
		if i > 0 && segment == "" {
			continue
		}
...
// addCommentSpans records the runs on one line of a comment.
//
// Classification is per LINE, so it catches an annotation trailing a declaration (AfterDeclComments) and one buried in
// a block comment just as well as a conventional doc block.
func addCommentSpans(byLine map[int][]theme.Span, line, col int, segment string, annotated bool) {
	...
	// The comment markers before the keyword stay prose; only the keyword itself is lifted out, and the value after it
	// returns to prose.
	if start > 0 {
		add(col, theme.SyntaxComment)
	}
