...
func TestRenderDiagnostics(t *testing.T) {
	t.Run("clean", func(t *testing.T) {
		if got, _ := renderDiagnostics("/work", nil, nil, 0, true); got != "(no diagnostics)" {
			t.Errorf("clean scan: got %q", got)
		}
	})

	t.Run("hard error", func(t *testing.T) {
		got, _ := renderDiagnostics("/work", codescan.ErrCodeScan, nil, 0, true)
	...
	t.Run("soft diagnostics", func(t *testing.T) {
		...
		got, _ := renderDiagnostics("/work", nil, diags, 0, true)
