...
const (
	// GutterAnchor marks a node with a source position of its OWN, so following
	// it lands exactly there rather than on an ancestor.
	GutterAnchor = '•'
...
// SetContent replaces the raw spec text, re-applying any active search. The
// cursor is CLAMPED, not reset: a rescan usually re-renders nearly the same
// document, and dropping the user back to line 0 on every save would make the
// live-reload loop unusable. Restoring it to the same NODE is the caller's job
// (see Model.refreshSpec).
func (p *Spec) SetContent(s string) {
	p.content = s
	p.cursor = clampSpec(p.cursor, 0, max(p.lineCount()-1, 0))
	p.render()
	p.revealCursor()
}
...
// Search sets the query, highlights matching lines, moves the cursor to the
// first match, and returns the match count. Putting the CURSOR on the match
// (rather than merely scrolling to it) means every cursor-driven action —
// follow, find-references, go-to-definition — acts on what you just searched
// for.
...
// CursorLine returns the 0-based content line the user is on. This is what
// every "the node under the cursor" question resolves against.
...
// SetCursor parks the cursor on the 0-based line, scrolling only as far as
// needed to keep it visible. The incremental primitive.
func (p *Spec) SetCursor(line int) {
	p.moveCursorTo(line)
	p.revealCursor()
}

// MoveCursor steps the cursor by delta, scrolling minimally. Used by the nav
// keys and the wheel, where a lurching viewport would be miserable.
func (p *Spec) MoveCursor(delta int) { p.SetCursor(p.cursor + delta) }

// JumpTo parks the cursor on the line and scrolls it to the VERTICAL CENTRE,
// clamped at the edges (design §6.1). The JUMP primitive: every cross-ref
// landing comes through here — follow-mode mirroring, `g` locate, the ctrl+f
// jump, F3 cycling, go-to-definition.
//
// Centring rather than the top-biased scroll search uses: in follow mode the
// target moves continuously, so a top bias pins it against whichever edge it
// entered from and makes it jitter, instead of letting it sit still while its
// surroundings slide past. For a one-shot jump it simply shows context on both
// sides of the destination.
...
//
// Focus also decides how the cross-ref line is painted: the driver pane keeps
// focus in follow mode (design §6.1), so "focused" and "is the driver" are the
// same bit. Re-render only on a focus TRANSITION — the spec can be thousands of
// lines and View runs on every message.
...
func clampSpec(v, lo, hi int) int {
	if v < lo {
		return lo
	}
	if v > hi {
...
// SetGutter installs the link markers, keyed by content line. A nil or empty
// map renders no gutter at all, so the pane costs no width until there is
// something to say (before the first scan, or with provenance switched off).
func (p *Spec) SetGutter(g map[int]rune) {
	p.gutter = g
	p.render()
}

// moveCursorTo clamps and sets the cursor, re-rendering only when it actually
// moved (the cursor style is baked into the viewport content).
func (p *Spec) moveCursorTo(line int) {
	line = clampSpec(line, 0, max(p.lineCount()-1, 0))
	if line == p.cursor {
		return
	}
	p.cursor = line
...
// cursorStyle is the whole-line style for the cursor: the strong bar when this
// pane drives, a muted tint when it is mirroring another pane.
...
// search highlight (per-substring), the cross-ref highlight (whole line) and
// the link gutter. The cross-ref line takes the whole-line style; search matches
// are still counted on it so n/N stays consistent.
//
// The gutter is prefixed AFTER highlighting, so the styles apply to the text the
// user searched for, not to the marker column.
func (p *Spec) render() {
	needle := ""
	if p.query != "" {
	...
	for i, ln := range lines {
		...
		// Precedence: cursor, then search, then syntax. The first two are
		// answers to "where am I" and "what did I ask for" — questions the user
		// posed — so they take the whole line rather than compete with colour
		// for it. One plain line reads fine; a line wearing three styles does
		// not.
		switch {
		case i == p.cursor:
...
// gutterFor renders line i's marker column, or blanks of the same width so the
// text stays aligned. Empty string when no gutter is installed.
...
func (p *Spec) scrollToMatch() {
	...
	// keep a little context above the match, rather than centring: when
	// stepping matches you want to see the ones that follow.
