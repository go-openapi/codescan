...
// lexicalSpans is what a `\t// in: formData` line tokenizes to: the comment
// markers, the keyword, then the value back in prose.
...
// The common case, and it is common by construction rather than by luck: a
// diagnostic and a lexical run both address a token, so they agree on where it
// starts.
...
// Not every diagnostic lands on a token boundary — codescan reports an invalid
// enum option at the space before the value. Opening a run there over-reaches
// rather than pointing somewhere false.
...
// Lexical spans are rebuilt when the BUFFER changes; marks change on every
// rescan. Mutating the input would make a rescan's marks accumulate on top of
// the previous scan's.
...
// A diagnostic on a line with no lexical runs still marks it — a blank or
// unclassified line can carry one.
