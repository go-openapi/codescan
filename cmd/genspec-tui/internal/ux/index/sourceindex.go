...
// AnchoredPointers returns every pointer that has an anchor of its OWN (not one
// inherited from an ancestor). Backs the spec pane's gutter: the caller maps
// each to its rendered line, marking the nodes whose source position is exact.
...
// AnchorLines returns the 1-based source lines in file that carry an anchor,
// i.e. the lines that produced a spec node. Backs the source viewer's gutter.
