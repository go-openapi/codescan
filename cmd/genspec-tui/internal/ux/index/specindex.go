...
// Indexes are the per-render products of a single lexer walk over the rendered
// spec: where each node is, where each $ref points, and what every token is.
// They are grouped because they share the walk — adding a fourth should not
// mean a fourth traversal of the same bytes.
type Indexes struct {
	Spec      *SpecIndex
	Refs      *RefIndex
...
// BuildJSONIndex builds the per-render indexes from indented JSON bytes in ONE
// lexer pass.
//
// The lexer reports, per token, its JSON pointer (RFC 6901 escaping handled for
// us) and its 1-based source line. The first token to report a pointer is the
// line that member is declared on; later repeats (its value, its closing
// delimiter) are the same node seen again. Reads the bytes the pane renders, so
// the ordered keys of spec.Swagger's MarshalJSON are preserved.
...
// BuildYAMLIndex is BuildJSONIndex over the YAML render. The YAML lexer emits
// the same logical token stream with the same RFC 6901 pointer escaping, so the
// indexes built from either render are interchangeable — only the lines differ.
