...
// refSpecJSON references /definitions/User from three places — a property, an
// array's items, and a response schema — plus one external ref that must be
// recorded but not resolvable, and a ref to a path key needing RFC 6901
// escaping. Indented exactly as json.MarshalIndent renders.
const refSpecJSON = `{
  "definitions": {
    "Team": {
      "properties": {
...
func TestRefIndex_FindsEveryLocalSite(t *testing.T) {
	...
	// Each site names the node HOLDING the $ref, not the $ref member itself —
	// that is the node the user is navigating to.
...
// The whole point of the index is the join: a site's target must be a pointer
// the SpecIndex can actually resolve to a line.
...
// YAML renders the same document at different lines; the pointers must match
// the JSON side exactly, so navigation survives a format toggle.
