...
//
// Soft diagnostics follow a one-line severity tally, one per row in source order, colored by severity; the selected row
// gets the whole-line highlight (for diagnostic→source navigation).
...
func Render(workdir string, scanErr error, diags []grammar.Diagnostic, selected int, focused bool) (string, int) {
	...
	for i, d := range diags {
		...
		if i == selected {
			...
			// Same rule as the spec pane and the source viewer: the strong bar means "you are driving this", the muted
			// tint means "this is where you were".
			// Two strong bars on screen at once make it ambiguous which pane a keypress will reach.
			if focused {
				row = theme.Selected().Render(row)
			} else {
				row = theme.Follower().Render(row)
			}
		}
		b.WriteString(row)
	}
...
// formatDiagnostic renders one diagnostic as "path:line:col severity: message [code]", with the severity label colored
// and the path made relative to workdir when it sits inside the scanned tree.
