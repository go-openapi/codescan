...
// key or scroll). f toggles it; any focus change or edit exits it.
//
...
// jumpDiagToSource opens the selected diagnostic's source line and MOVES FOCUS there.
//
// The one-shot counterpart to f follow mode, matching g from the tree and Enter in the spec pane.
//
...
// driveDiagFollowers mirrors BOTH followers to the selected diagnostic.
//
...
//
// This goes through the SAME nearest-anchor walk the source pane's own f uses,
// rather than anything diagnostic-specific: "what did this line turn into" has one answer,
// and it should not depend on which pane asked.
//
...
// refreshSpec re-renders the spec pane and rebuilds its line-to-pointer index.
//
// The body comes from the stored JSON or YAML, per the active format toggle.
// The index is the spec-side half of the cross-ref linker.
...
//
// dir is +1 for the next site and -1 for the previous, wrapping at the ends.
//
...
//
// It enters at the first site for a forward step, and at the last for a backward one.
//
...
// rebuildGutters recomputes the link markers for both panes.
//
// This is the discoverability layer: it says which lines actually lead somewhere, now that both indexes exist.
//
...
// locateInSpec jumps the spec pane to the first node a source file produced.
//
// Position-backed, through the SourceIndex. The node is highlighted and the spec takes focus.
//
...
// toggleFollow turns the given follow mode on, or off when it is already active.
//
// Driving is from the current pane, with an immediate first sync on entry.
...
// exitFollow leaves follow mode and drops the spec follower highlight.
//
// The source nav line is the viewer's own cursor, so it stays.
...
//
// Runs after every key or scroll.
...
// driveSpecToSource mirrors the source follower to the spec node at the top of the viewport.
//
// Neither focus nor the spec scroll moves: the user drives those.
//
