...
func TestRenderDiagnostics_SelectedLine(t *testing.T) {
	...
	_, line := renderDiagnostics("/w", nil, diags, 1, true)
	assert.Equal(t, 2, line)
}

// Spotted in a screenshot: the spec cursor and the selected diagnostic were
// both drawn with the strong bar, so two panes appeared to be driving at once.
...
func TestDiag_SelectionDimsWhenUnfocused(t *testing.T) {
	...
	focused, _ := renderDiagnostics("", nil, diags, 1, true)
	unfocused, _ := renderDiagnostics("", nil, diags, 1, false)

	assert.NotEqual(t, focused, unfocused,
		"the selected row must look different depending on whether the pane drives")
	assert.Equal(t, stripANSI(focused), stripANSI(unfocused),
