...
func renderDiagnostics(workdir string, scanErr error, diags []grammar.Diagnostic, selected int, focused bool) (string, int) {
	...
	for i, d := range diags {
		...
		if i == selected {
			...
			// Same rule as the spec pane and the source viewer (§6.5): the
			// strong bar means "you are driving this", the muted tint means
			// "this is where you were". Two strong bars on screen at once make
			// it ambiguous which pane a keypress will reach.
			if focused {
