...
func scanned(t *testing.T, dir string, argv ...string) map[string]any {
	...
	argv = append([]string{"-workdir", dir, "-quiet"}, argv...)
...
func TestRunWritesToAFile(t *testing.T) {
	...
	stdout, _, err := exec(t, "-workdir", petstore(t), "-quiet", "-output", path, "./...")
	require.NoError(t, err)
...
func TestRunInfersYAMLFromTheOutputName(t *testing.T) {
	...
	_, _, err := exec(t, "-workdir", petstore(t), "-quiet", "-output", path, "./...")
	require.NoError(t, err)
...
func TestRunCompactDropsTheIndentation(t *testing.T) {
	...
	stdout, _, err := exec(t, "-workdir", petstore(t), "-quiet", "-compact", "./...")
	require.NoError(t, err)
...
func TestRunRefusesADirectoryAsInput(t *testing.T) {
	...
	_, _, err := exec(t, "-workdir", petstore(t), "-quiet", "-input", t.TempDir(), "./...")
...
func TestRunValidateFailsOnAnInvalidDocument(t *testing.T) {
	...
	stdout, stderr, err := exec(t, "-workdir", unannotated(t), "-validate", "-color=never", "./...")
...
func TestRunFailsOnTheThresholdItWasGiven(t *testing.T) {
	...
	_, _, err := exec(t, "-workdir", unannotated(t), "-fail-on=warning", "-color=never", "./...")
...
func TestRunIsSilentByDefaultOnACleanScan(t *testing.T) {
	...
	_, stderr, err := exec(t, "-workdir", petstore(t), "-color=never", "./...")
...
func TestRunQuietSaysNothing(t *testing.T) {
	...
	stdout, stderr, err := exec(t, "-workdir", unannotated(t), "-quiet", "-verbose", "./...")
...
func TestRunReportsPositionsRelativeToTheWorkingDirectory(t *testing.T) {
	...
	_, stderr, err := exec(t, "-workdir", unannotated(t), "-color=never", "./...")
...
func TestRunRejectsAnUnreadableWorkdir(t *testing.T) {
	...
	_, _, err := exec(t, "-workdir", filepath.Join(t.TempDir(), "nowhere"), "-quiet", "./...")
