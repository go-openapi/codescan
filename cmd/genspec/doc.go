...
//	genspec -workdir ./api ./...
//
// It is the ordinary, native way to run codescan: everything the library can be told is a flag,
// the document goes to standard output or to -output, and what the scan observed goes to standard error
// as colored diagnostics. See -h for the whole surface.
//
// # The family
//
// Three commands run the same scan:
//
//	genspec       native, colored diagnostics, YAML, merges into an existing document, validates what it produced
//	genspec-tui   the same scan, live, with the source and the document side by side
//	genspec-wasi  the same scan with no dependency beyond the library, so it cross-compiles to WebAssembly and runs
//	              under a WASI runtime with no Go toolchain installed.
//	              It speaks a machine-readable envelope (-format=json) carrying diagnostics and cross-references.
//
// # Configuration file
//
// Anything that can be a flag can be preset in a .codescan.yaml, found by searching upwards from
// wherever the command is run. Keys are grouped into sections, and inside a section a key is the flag it sets,
// spelled exactly as on the command line:
//
//	scan:
//	  workdir: ./api
//	emit:
...
//	  format: yaml
//
// Anything typed on the command line wins over the file, including a flag typed with the value it already had.
// Use -config, or -c, to name a particular file; -no-config reads none.
//
